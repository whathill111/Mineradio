...
function getAdaptiveRenderFps(now) {
  ...
  if (fixedFps !== null) {
    // "Follow display" is useful while music or direct interaction is
    // animating the scene. At rest it needlessly redraws an unchanged WebGL
    // frame at 60/120/144 Hz, so retain VSync responsiveness and idle at 20.
    if (fixedFps === 0) return visibleMotionFollowVsync(now) ? 0 : 20;
    return fixedFps;
  }
...
var mainFrameGates = {
  ...
  backgroundMotion: createFrameGate('main.backgroundMotion', 30),
...
function animate() {
  ...
  var backgroundMotionStepDt = consumeFrameGate(mainFrameGates.backgroundMotion, now, dt, 30, false, 'background-motion');
  if (backgroundMotionStepDt > 0 && typeof tickCustomBackgroundMotion === 'function') {
    tickCustomBackgroundMotion(backgroundMotionStepDt, {
      time: uniforms.uTime.value,
      bass: bass,
      mid: mid,
      treble: treble,
