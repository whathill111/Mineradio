...
var customBackgroundMotionState = {
  x: 0,
  y: 0,
  scale: 1,
  brightness: 1,
...
function customBackgroundMotionLabel(mode) {
  mode = normalizeBackgroundMotionMode(mode);
  if (mode === 'off') return '\u5173\u95ed';
  if (mode === 'audio') return '\u97f3\u4e50\u5f8b\u52a8';
  return '\u7535\u5f71\u6f02\u79fb';
}
function customBackgroundMotionFrame(mode, timeSeconds, audioFrame) {
  mode = normalizeBackgroundMotionMode(mode);
  var time = Math.max(0, Number(timeSeconds) || 0);
  var frame = audioFrame && typeof audioFrame === 'object' ? audioFrame : {};
  if (mode === 'off') return { x: 0, y: 0, scale: 1, brightness: 1, saturation: 1 };
  var driftX = Math.sin(time * 0.071) * 0.82 + Math.sin(time * 0.029 + 1.7) * 0.44;
  var driftY = Math.cos(time * 0.053 + 0.8) * 0.58 + Math.sin(time * 0.021) * 0.30;
  var cinemaScale = 1.031 + (Math.sin(time * 0.043 - 0.6) + 1) * 0.006;
  if (mode === 'cinema') {
    return { x: driftX, y: driftY, scale: cinemaScale, brightness: 1, saturation: 1 };
  }
  var energy = clampRange(Number(frame.energy) || 0, 0, 1);
  var bassDrive = clampRange(Number(frame.bass) || 0, 0, 1);
  var beatDrive = clampRange(Number(frame.beat) || 0, 0, 1);
  var trebleDrive = clampRange(Number(frame.treble) || 0, 0, 1);
  return {
    x: driftX * 0.92,
    y: driftY * 0.92,
    scale: cinemaScale + energy * 0.010 + bassDrive * 0.007 + beatDrive * 0.012,
    brightness: 1 + energy * 0.025 + beatDrive * 0.018,
    saturation: 1 + energy * 0.075 + trebleDrive * 0.035
  };
}
function customBackgroundMotionShouldReduce() {
  try {
...
function applyCustomBackgroundMotionVars(frame, active) {
  frame = frame || { x: 0, y: 0, scale: 1, brightness: 1, saturation: 1 };
  var css = [
    frame.x.toFixed(3) + '%',
    frame.y.toFixed(3) + '%',
    frame.scale.toFixed(4),
    frame.brightness.toFixed(4),
    frame.saturation.toFixed(4)
  ].join('|');
  if (css !== customBackgroundMotionState.lastCss) {
    var root = document.documentElement;
    root.style.setProperty('--custom-bg-motion-x', frame.x.toFixed(3) + '%');
    root.style.setProperty('--custom-bg-motion-y', frame.y.toFixed(3) + '%');
    root.style.setProperty('--custom-bg-motion-scale', frame.scale.toFixed(4));
    root.style.setProperty('--custom-bg-motion-brightness', frame.brightness.toFixed(4));
    root.style.setProperty('--custom-bg-motion-saturation', frame.saturation.toFixed(4));
    customBackgroundMotionState.lastCss = css;
  }
  if (document.body) document.body.classList.toggle('custom-bg-motion-active', !!active);
}
function tickCustomBackgroundMotion(dt, frame) {
  frame = frame || {};
  var mode = normalizeBackgroundMotionMode(fx && fx.backgroundMotionMode);
  var media = customBackgroundActiveMedia();
  var active = mode !== 'off'
    && !!media
    && !document.hidden
    && !customBackgroundMotionShouldReduce()
    && !(document.body && document.body.classList.contains('wallpaper-engine-active'));
  var target = active
    ? customBackgroundMotionFrame(mode, frame.time, frame)
    : customBackgroundMotionFrame('off', 0, null);
  var seconds = clampRange(Number(dt) || 0, 0, 0.25);
  var positionEase = 1 - Math.exp(-seconds * 3.2);
  var toneEase = 1 - Math.exp(-seconds * (mode === 'audio' ? 8.5 : 4.5));
  customBackgroundMotionState.x += (target.x - customBackgroundMotionState.x) * positionEase;
  customBackgroundMotionState.y += (target.y - customBackgroundMotionState.y) * positionEase;
  customBackgroundMotionState.scale += (target.scale - customBackgroundMotionState.scale) * toneEase;
  customBackgroundMotionState.brightness += (target.brightness - customBackgroundMotionState.brightness) * toneEase;
  customBackgroundMotionState.saturation += (target.saturation - customBackgroundMotionState.saturation) * toneEase;
  applyCustomBackgroundMotionVars(customBackgroundMotionState, active);
  return {
    active: active,
    mode: mode,
    x: customBackgroundMotionState.x,
    y: customBackgroundMotionState.y,
    scale: customBackgroundMotionState.scale,
    brightness: customBackgroundMotionState.brightness,
    saturation: customBackgroundMotionState.saturation
  };
}
function updateCustomBackgroundMotionControls() {
  var mode = normalizeBackgroundMotionMode(fx && fx.backgroundMotionMode);
  if (fx) fx.backgroundMotionMode = mode;
  var value = document.getElementById('bg-motion-value');
  if (value) value.textContent = customBackgroundMotionLabel(mode);
  document.querySelectorAll('#background-motion-seg [data-bg-motion]').forEach(function (button) {
    var active = button.getAttribute('data-bg-motion') === mode;
    button.classList.toggle('active', active);
    button.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
}
function setCustomBackgroundMotionMode(mode, silent) {
  fx.backgroundMotionMode = normalizeBackgroundMotionMode(mode);
  updateCustomBackgroundMotionControls();
  if (fx.backgroundMotionMode === 'off') tickCustomBackgroundMotion(0.25, { time: 0 });
  saveLyricLayout({ user: true, reason: 'backgroundMotionMode' });
  if (!silent) showToast('\u80cc\u666f\u52a8\u6001: ' + customBackgroundMotionLabel(fx.backgroundMotionMode));
}
