...
function songSourceLabel(song) {
  if (!song) return '未知';
  if (song.provider === 'open' || song.source === 'open' || song.source === 'open-audio' || song.type === 'open' || song.archiveId) return '开放音频';
...
function openTrackDetailModal(type, songOverride) {
  ...
  if (type === 'album') {
    ...
    var openAudioAttribution = songSourceLabel(song) === '开放音频'
      ? '<div class="detail-section open-audio-attribution"><div class="detail-section-head"><div class="detail-section-title">开放授权信息</div></div>' +
        '<div class="detail-chip-row"><span class="detail-chip">' + escHtml(song.licenseLabel || '开放授权') + '</span>' +
        (song.sourceUrl ? '<a class="detail-chip detail-link" href="' + escHtml(song.sourceUrl) + '" target="_blank" rel="noopener noreferrer">查看来源</a>' : '') +
        (song.licenseUrl ? '<a class="detail-chip detail-link" href="' + escHtml(song.licenseUrl) + '" target="_blank" rel="noopener noreferrer">查看许可</a>' : '') +
        '</div></div>'
      : '';
    body.innerHTML =
      '<div class="detail-hero">' + coverHtml +
...
function songAccountProvider(song) {
  ...
  if (song.provider === 'open' || song.source === 'open' || song.source === 'open-audio' || song.type === 'open' || song.archiveId) return 'open';
  if (song.provider === 'spotify' || song.source === 'spotify' || song.type === 'spotify' || song.spotifyId || song.spotifyUri) return 'spotify';
