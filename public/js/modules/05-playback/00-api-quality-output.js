...
function readPlaybackQualityPreference() {
  var fallback = {
    ...
    open: PLAYBACK_QUALITY_DEFAULTS.open
  };
  try {
    ...
    if (raw.trim().charAt(0) !== '{') {
      ...
      return {
        ...
        open: normalizePlaybackQualityForProvider(fallback.open, 'open')
      };
    }
    ...
    return {
      ...
      open: normalizePlaybackQualityForProvider(parsed.open || fallback.open || 'standard', 'open')
    };
  } catch (e) {
    return fallback;
  }
...
function updatePlaybackQualityUi() {
  ...
  var qualityProviderTitle = provider === 'open' ? '开放音频: ' : (provider === 'spotify' ? 'Spotify 匹配源: ' : (provider === 'qishui' ? '汽水音质: ' : (provider === 'qq' ? 'QQ 音质: ' : (provider === 'kugou' ? '酷狗音质: ' : '网易云音质: '))));
