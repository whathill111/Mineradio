...
async function resolveAlbumGaplessPlaybackData(song) {
  ...
  if (playbackProvider === 'open') {
    return apiJson('/api/open/song/url?archiveId=' + encodeURIComponent(song.archiveId || '') +
      '&file=' + encodeURIComponent(song.archiveFile || ''), { timeoutMs: 10000 });
  }
...
async function playQueueAt(idx, opts) {
  ...
  try {
    ...
    try {
      ...
      var isOpenPlayback = playbackProvider === 'open';
      ...
      if (albumGaplessHandoff) {
        ...
        data = await apiJson('/api/spotify/song/url?id=' + encodeURIComponent(song.id || song.providerSongId || song.spotifyId || '') +
          ...
          '&uri=' + encodeURIComponent(song.spotifyUri || song.uri || '') +
          qualityParam, { timeoutMs: 9000 });
      } else if (isOpenPlayback) {
        data = await apiJson('/api/open/song/url?archiveId=' + encodeURIComponent(song.archiveId || '') +
          '&file=' + encodeURIComponent(song.archiveFile || ''), { timeoutMs: 10000 });
      } else {
