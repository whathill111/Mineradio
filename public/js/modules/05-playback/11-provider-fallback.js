...
function showSourceFallbackNotice(title, body) {
  ...
  if (stack) {
    var signature = String(title || '自动换源') + '\n' + String(body || '');
    var duplicate = Array.prototype.slice.call(stack.children || []).find(function (item) {
      return item && item.dataset && item.dataset.noticeSignature === signature && !item.__mineradioRemoving;
    });
    if (duplicate) {
      stack.insertBefore(duplicate, stack.firstChild || null);
    ...
    card.dataset.noticeSignature = signature;
    ...
    // Enforce the hard cap synchronously. removeSourceFallbackCard animates
    // for 260 ms, so using it in this while loop leaves children.length
    // unchanged and loops forever as soon as the fifth notice arrives.
...
var SOURCE_FALLBACK_DIRECT_PROVIDERS = ['netease', 'qq', 'kugou', 'open'];
...
async function searchAlternatePlatformSong(song, requestedTarget, recovery) {
  ...
  var url = target === 'qq'
    ...
    : (target === 'kugou'
      ...
      : (target === 'open'
        ? '/api/open/search?keywords=' + encodeURIComponent(query) + '&limit=8'
