...
var SEARCH_HISTORY_MODES = ['song', 'netease', 'qq', 'kugou', 'qishui', 'spotify', 'open', 'podcast'];
...
function updateSearchModeTabs() {
  ...
  var openBtn = document.getElementById('search-mode-open');
  ...
  if (openBtn) {
    openBtn.classList.toggle('active', searchMode === 'open');
    openBtn.setAttribute('aria-selected', searchMode === 'open' ? 'true' : 'false');
  }
  if (podcastBtn) {
  ...
  if ($input && searchMode === 'open') $input.placeholder = '搜索开放授权音乐（英文关键词更丰富）...';
  requestAnimationFrame(updateSearchPillGlassDisplacementMap);
}
function setSearchMode(mode) {
  mode = (mode === 'podcast' || mode === 'netease' || mode === 'qq' || mode === 'kugou' || mode === 'qishui' || mode === 'spotify' || mode === 'open') ? mode : 'song';
...
function songProviderKey(song) {
  if (song && (song.provider === 'open' || song.source === 'open' || song.source === 'open-audio' || song.type === 'open' || song.archiveId)) return 'open';
...
function songSourceTagHtml(song, opts) {
  ...
  var key = /^(netease|qq|kugou|qishui|spotify|open)$/.test(String(rawKey || '')) ? String(rawKey) : songProviderKey(song);
  var label = key === 'qq' ? 'QQ' : (key === 'kugou' ? 'KG' : (key === 'qishui' ? 'QS' : (key === 'spotify' ? 'SP' : (key === 'open' ? 'OA' : 'NE'))));
  if (opts.switcher) {
...
function controlSourceSearchUrl(provider, query) {
  ...
  if (provider === 'open') return '/api/open/search?keywords=' + encodeURIComponent(query) + '&limit=8';
...
function searchResultMetaText(song) {
  ...
  if (songProviderKey(song) === 'open') bits.push((song.licenseLabel || '开放授权') + ' · 免费直放');
...
var MUSIC_SEARCH_PROVIDER_ORDER = ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'open'];
...
function searchProviderCanSearch(provider) {
  ...
  return provider === 'netease' || provider === 'qq' || provider === 'kugou' || provider === 'qishui' || provider === 'open';
}
function searchModeProvider(mode) {
  return mode === 'netease' || mode === 'qq' || mode === 'kugou' || mode === 'qishui' || mode === 'spotify' || mode === 'open' ? mode : '';
}
...
function searchProviderUrl(provider, q, limit, offset) {
  ...
  if (provider === 'open') return '/api/open/search?keywords=' + encodeURIComponent(q) + suffix;
...
function mergeSongSearchResults(neteaseSongs, qqSongs, kugouSongs, qishuiSongs, spotifySongs, openSongs, limit, q) {
  var out = [];
  var providerSeen = {};
  var canonicalSeen = {};
  ...
  (openSongs || []).forEach(function (song, i) { push(song, i); });
...
async function fetchMusicSearchResults(q, mode, previousPages) {
  ...
  var pageLimitByProvider = { netease: 18, qq: 12, kugou: 12, qishui: 12, spotify: 10, open: 8 };
  ...
  var songsByProvider = { netease: [], qq: [], kugou: [], qishui: [], spotify: [], open: [] };
