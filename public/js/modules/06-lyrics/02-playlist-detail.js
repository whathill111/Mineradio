...
async function openPlaylistPanelDetail(provider, pid, title) {
  ...
  if (playlistPanelDetailState.key === key) {
    // A double click while the first request is in flight used to cancel the
    // request, clear the detail DOM, and immediately start another render.
    // Treat that as the same open action until the first page settles.
    if (playlistPanelDetailState.loading || playlistPanelDetailState.loadingMore) return;
