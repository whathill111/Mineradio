...
function isPlaylistEdgeTrigger(ex, ey, H, eventTarget) {
  // The planner and pet import controls live in the same left edge zone as
  // the peek playlist.  They must win hit-testing while the pointer is over
  // their complete card, including the small gap between controls.
  var protectedTarget = eventTarget && eventTarget.closest && eventTarget.closest('#study-planner,#study-pet-settings,#upload-panel,#upload-tip');
  var protectedPlanner = document.getElementById('study-planner');
  if (protectedTarget || (protectedPlanner && protectedPlanner.getBoundingClientRect)) {
    var rect = protectedPlanner && protectedPlanner.getBoundingClientRect();
    if (rect && ex >= rect.left - 18 && ex <= rect.right + 18 && ey >= rect.top - 18 && ey <= rect.bottom + 18) {
