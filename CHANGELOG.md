...
- 双显卡笔记本默认使用省电 GPU（核显）渲染，不再全程点亮独显，风扇显著安静；安装器会自动写入省电 GPU 首选项，需要独显可在 Windows「显示卡设置」改回。
- 歌词正文纹理增加字节预算与滑动窗口，长歌播放期间 GPU 内存不再无限增长，停止播放后全额回落。
- 修复应用内更新与打包配置仍指向原作者仓库的问题，现在正确指向本仓库。
