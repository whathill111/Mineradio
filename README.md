...
[下载 Mineradio 2.1.1 — GitHub Release](https://github.com/whathill111/Mineradio/releases/tag/v2.1.1)

安装时只需要下载并运行 `Mineradio-2.1.1-Setup.exe`。不要把 `.blockmap`、`latest.yml` 或 `win-unpacked` 当成正式安装包。

## 下载或安装被拦截怎么办

小众 Electron 桌面软件、未签名安装包有时会被浏览器、Windows Defender 或 SmartScreen 提示风险。请先确认安装包来自本仓库的 GitHub Release，文件名是 `Mineradio-2.1.1-Setup.exe`。
...
状态：Mineradio 2.1.1 正式版。

> 安全提示：`v1.0.10` 及更早旧安装包不再建议继续安装或传播。请使用本仓库 GitHub Release 提供的 `Mineradio-2.1.1-Setup.exe`。
...
正式分发以 `Mineradio-2.1.1-Setup.exe` 为准，不建议直接使用 `win-unpacked` 目录。安装包会创建桌面快捷方式。
...
已经安装过旧版本的用户可直接运行 `Mineradio-2.1.1-Setup.exe` 完成更新。软件内更新入口只会打开浏览器下载页，不会在客户端内下载或应用补丁。
